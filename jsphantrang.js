...
(function() {
    ...
    // 👉 TÚ TỰ VIẾT TAY DANH SÁCH FILE VÀO MẢNG NÀY NHÉ:
    const files = [
    ...
    // Lấy tên file đang mở hiện tại trên trình duyệt
    ...
    // Tìm vị trí của file đang xem trong mảng
    const activeFileIndexInList = files.indexOf(currentFileName);

    // Tính trang hiện tại dựa trên file đang active
    let currentPage = 1;
    ...
    function renderPage(page) {
        ...
        if (TOTAL_FILES === 0) {
            container.innerHTML = '<p style="color:#888; width:100%; text-align:center; padding:20px;">Không có dữ liệu file.</p>';
        ...
        for (let i = start; i < end; i++) {
            ...
            const isActive = (fileName === currentFileName) ? 'active' : '';
            
            // Tạo tên hiển thị trên nút (Bỏ đuôi .html cho gọn)
            const labelText = fileName.replace(/\.html$/i, '');

            container.innerHTML += `
                <div class="video-item">
                    <a href="${fileName}">
                        <div class="num-box ${isActive}">${labelText}</div>
                    </a>
                </div>
            `;
        }
